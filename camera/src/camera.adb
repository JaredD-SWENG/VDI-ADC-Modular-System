...
package body Camera is
   ...
   function Load_Video (Dir : String) return Video is
      Nbr_Frames : constant Natural := Get_Frame_End_Index;
      Vid : Video;
   begin
      for I in 0 .. Nbr_Frames - 1 loop
         Put_Line (Dir & "\" & "frame_" & Zero_Pad (I, 4) & ".qoi");
         Vid.Append (CV_Ada.IO_Operations.Load_QOI (Dir & "\" & "frame_" & Zero_Pad (I, 4) & ".qoi"));
      end loop;
      return Vid;
   end Load_Video;
   ...
   is
      Search_Filter : constant Filter_Type := (Directory     => False,
                                               Ordinary_File => True,
                                               Special_File  => False);
   begin
