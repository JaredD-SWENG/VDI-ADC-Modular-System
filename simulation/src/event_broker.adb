      ...
      loop
         ...
         begin
            ...
            if not Path_Processing and not Queue_Empty then
               ...
               GUI_Functions.AddConsoleText ("Event Broker: Started Path Planning");
            end if;
            ...
            if Path_Processing and Queue_Has_Higher_Priority then
               ...
               GUI_Functions.AddConsoleText ("Event Broker: Killed Path Planning for higher priority event");
            end if;

            -- Reset state when queue becomes empty
            if Path_Processing and Queue_Empty then
               Path_Processing := False;
               GUI_Functions.AddConsoleText ("Event Broker: Path Planning completed (queue empty)");
            end if;
         end;
