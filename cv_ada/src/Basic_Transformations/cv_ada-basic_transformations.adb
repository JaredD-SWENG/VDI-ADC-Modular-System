...
begin
   ...
   begin
      ...
      -- Create a new data array with the correct size for the ROI
      declare
         New_Data : Storage_Array_Access := 
           new Storage_Array(1 .. Width * Height * Desc.Channels);
      begin
         -- Copy the ROI data to the new array
         New_Data.all := Temp;
         
         -- Free the old data array
         Free(Data);
         
         -- Update the Input record with the new data and dimensions
         Input.Data := New_Data;
         Input.Desc.Width := Width;
         Input.Desc.Height := Height;
      end;
   end;
...
is
   ...
   -- Temporary array to store original image data
...
procedure Adjust_Brightness
   (Input : in out Input_Data; Brightness : Integer;
...
procedure Adjust_Contrast
   (Input : in out Input_Data; Factor : Float)
is
   -- Access the input data and descriptor
   Data : Storage_Array_Access := Input.Data;
   Desc : QOI.QOI_Desc := Input.Desc;
...
begin
   ...
   --   Desc - QOI descriptor containing image metadata
   ------------------------------------------------------------------------------
   procedure Invert_Colors (Input : in out Input_Data) is
      Data : Storage_Array_Access := Input.Data;
      Desc : QOI.QOI_Desc := Input.Desc;
      -- Number of channels per pixel (3 for RGB, 4 for RGBA)
      Pixel_Size : constant Storage_Count := Storage_Count (Desc.Channels);
   ...
   ------------------------------------------------------------------------------
procedure Sharpen_Image (Input : in out Input_Data)
is
   -- Access the input data and descriptor
   Data : Storage_Array_Access := Input.Data;
   Desc : QOI.QOI_Desc := Input.Desc;
                  ...
                  for Kx in -1 .. 1 loop
                     declare
                        ...
                        Src_Pos : constant Storage_Count :=
                          Storage_Count(((Src_Y * Width) + Src_X) * Pixel_Size + C + 1);
                     begin
                        Sum := Sum + Integer(Data(Src_Pos)) * Kernel(Ky, Kx);
                     end;
                  end loop;
               end loop;
